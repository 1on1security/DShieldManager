...
SELECT sensorName AS Sensor, url AS "Bottom URL", sip AS "Source IP", url_count AS Count
FROM (
  SELECT sensorName, sip, url_count, url,
...
html_table = "<!DOCTYPE html><html><head><title>Webhoneypot.json Bottom 5 URLs per Sensor</title><style>body {background: url('images/blueCamo.jpg') no-repeat center center fixed;background-size: cover;</style></head><body>"
html_table += "<h1 style='text-align: left; font-family: Verdana; color: white;'>Webhoneypot.json Bottom 5 URLs per Sensor</h1>"
...
for i in range(0, len(sensor_rows), sensors_per_column):
    ...
    for j in range(sensors_per_column):
        if i + j < len(sensor_rows):
            ...
            html_table += "<tr><th style='font-family: Verdana;'>Sensor</th><th style='font-family: Verdana;'>Bottom URL</th><th style='font-family: Verdana;'>Source IP</th><th style='font-family: Verdana;'>Count</th></tr>"
...
file_path = "/var/www/html/bottom5URLs.html"
